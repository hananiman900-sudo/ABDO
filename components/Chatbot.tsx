                        ...
                        {offers.map(o => (
                            <div key={o.id} className="border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="p-3">
                                    ...
                                    <button onClick={() => onBookOffer && onBookOffer(o)} className="w-full bg-black text-white py-2 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1 active:scale-95 transition-transform">
...
const Chatbot: React.FC<{ currentUser: AuthenticatedUser | null; onOpenAuth: () => void; onDiscover: () => void; onToggleNav: (hidden: boolean) => void; onOpenNotifications: () => void; initialProvider?: any; initialMessage?: string }> = ({ currentUser, onOpenAuth, onDiscover, onToggleNav, onOpenNotifications, initialProvider, initialMessage }) => {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const { data: adsData } = await supabase.from('provider_ads').select('provider_id, created_at').eq('is_active', true).order('created_at', { ascending: false });
            const lastAdMap: Record<number, string> = {};
            if (adsData) { adsData.forEach(ad => { if (!lastAdMap[ad.provider_id]) { lastAdMap[ad.provider_id] = ad.created_at; } }); }
            let processedProviders = pData || [];
            processedProviders = processedProviders.map(p => ({ ...p, last_ad_at: lastAdMap[p.id] || null }));
            processedProviders.sort((a, b) => { if (a.last_ad_at && b.last_ad_at) return new Date(b.last_ad_at).getTime() - new Date(a.last_ad_at).getTime(); if (a.last_ad_at) return -1; if (b.last_ad_at) return 1; return (b.visits_count || 0) - (a.visits_count || 0); });
            ...
            if (currentUser && processedProviders) { calculateUnreadBadges(currentUser.id); }
        }
        init();
    }, [currentUser]);

    useEffect(() => { if(initialProvider) { openChat(initialProvider); if(initialMessage) { setTimeout(() => handleSend(initialMessage), 800); } } }, [initialProvider, initialMessage]);
    ...
    const calculateUnreadBadges = async (userId: number) => {
        ...
        allAds.forEach(ad => { if (!viewedAdIds.has(ad.id)) { counts[ad.provider_id] = (counts[ad.provider_id] || 0) + 1; } });
    ...
    useEffect(() => { scrollToBottom(); }, [messages, view, suggestions, isRecording, audioBlob]);
    ...
    useEffect(() => {
        if (view === 'CHAT' && currentUser) {
            const loadHistory = async () => {
                ...
                if (selectedChat) query = query.eq('provider_id', selectedChat.id); else query = query.is('provider_id', null);
                ...
                if (data) {
                    const formatted: Message[] = data.map((msg: any) => {
                        ...
                        if (msg.role === 'BOT' && msg.text && msg.text.startsWith('{"appointmentId"')) {
                             try { bookingDetails = JSON.parse(msg.text); isComponent = true; return { role: Role.BOT, text: bookingDetails.message || "Booking Details", bookingDetails: bookingDetails, isComponent: true }; } catch (e) { return { role: Role.BOT, text: msg.text }; }
                        }
                        return { role: msg.role === 'USER' ? Role.USER : Role.BOT, text: msg.text || '', imageUrl: msg.image_url, }
                    });
                    setMessages(formatted);
                } else { setMessages([]); }
            };
            loadHistory();
        } else if (view === 'CHAT' && !currentUser) { setMessages([]); }
        ...
        if(view === 'CHAT') {
            if (selectedChat) {
                ...
                const fetchAndMarkAds = async () => {
                    const { data: ads } = await supabase.from('provider_ads').select('*').eq('provider_id', selectedChat.id).eq('is_active', true).order('created_at', { ascending: true }); 
                    if (ads && ads.length > 0 && currentUser) {
                        const { data: views } = await supabase.from('ad_views').select('ad_id').eq('user_id', currentUser.id).in('ad_id', ads.map(a => a.id));
                        const viewedIds = new Set(views?.map(v => v.ad_id) || []);
                        const unreadAds = ads.filter(a => !viewedIds.has(a.id));
                        if (unreadAds.length > 0) { const inserts = unreadAds.map(ad => ({ user_id: currentUser.id, ad_id: ad.id })); await supabase.from('ad_views').insert(inserts); setUnreadCounts(prev => ({...prev, [selectedChat.id]: 0})); }
                        setMessages(prev => { const existingAdIds = new Set(prev.filter(m => m.isWelcomeAd && (m as any)._adId).map(m => (m as any)._adId)); const newAdMessages = ads.filter(ad => !existingAdIds.has(ad.id)).map(ad => ({ role: Role.BOT, text: ad.message, imageUrl: ad.image_url, isWelcomeAd: true, _adId: ad.id })); return [...prev, ...newAdMessages]; });
                    }
                };
                setTimeout(fetchAndMarkAds, 500);
            } else { setSuggestions(['🏥 أبحث عن طبيب', '🔧 أحتاج سباك', '🏠 سمسار عقارات']); }
        }
    }, [view, selectedChat, currentUser]);

    const openChat = (provider: any | null) => { setSelectedChat(provider); setView('CHAT'); setShowProfile(false); setShowBooking(false); };
    ...
    const handleChipClick = async (text: string) => { if (text === '📅 حجز موعد') { setPreSelectedOffer(null); setShowBooking(true); return; } handleSend(text); }

    const handleSend = async (manualText?: string) => {
        const textToSend = manualText || input;
        if (audioBlob) {
            setMessages(prev => [...prev, { role: Role.USER, text: "🎤 رسالة صوتية (Audio)" }]);
            setAudioBlob(null); setIsLoading(true);
            try {
                const reader = new FileReader();
                reader.readAsDataURL(audioBlob);
                reader.onloadend = async () => {
                    const base64Audio = (reader.result as string).split(',')[1];
                    const responseText = await getChatResponse([], "", language, undefined, { base64: base64Audio, mimeType: 'audio/mp3' }, currentUser?.id, currentUser?.name, selectedChat);
                    setMessages(prev => [...prev, { role: Role.BOT, text: responseText }]);
                    setIsLoading(false);
                };
            } catch (e) { setMessages(prev => [...prev, { role: Role.BOT, text: "Audio Error" }]); setIsLoading(false); }
            return;
        }
        ...
        setMessages(prev => [...prev, userMsg]); setInput(''); setPreview(null); setSuggestions([]); setIsLoading(true);
        if (currentUser) { await supabase.from('chat_history').insert({ user_id: currentUser.id, provider_id: selectedChat ? selectedChat.id : null, role: 'USER', text: userMsg.text, image_url: userMsg.imageUrl }); }
        try {
            const history = messages.filter(m => !m.isComponent && !m.isWelcomeAd).map(m => ({ role: m.role === Role.USER ? 'user' : 'model', parts: [{ text: m.text }] }));
            const responseText = await getChatResponse(history, userMsg.text, language, userMsg.imageUrl ? { base64: userMsg.imageUrl.split(',')[1], mimeType: 'image/jpeg' } : undefined, undefined, currentUser?.id, currentUser?.name, selectedChat);
            let displayText = responseText; const suggestionMatch = responseText.match(/SUGGESTIONS\|(.*)/); if (suggestionMatch) { displayText = responseText.replace(suggestionMatch[0], '').trim(); const rawOptions = suggestionMatch[1].split('|'); setSuggestions(rawOptions.map(o => o.trim()).filter(o => o.length > 0)); }
            let botMsg: Message = { role: Role.BOT, text: displayText };
            setMessages(prev => [...prev, botMsg]);
            if (currentUser) { await supabase.from('chat_history').insert({ user_id: currentUser.id, provider_id: selectedChat ? selectedChat.id : null, role: 'BOT', text: botMsg.text }); }
        } catch(e) { setMessages(prev => [...prev, { role: Role.BOT, text: t('errorMessage') }]); } finally { setIsLoading(false); }
    };

    const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if(file) { const reader = new FileReader(); reader.onload = () => setPreview(reader.result as string); reader.readAsDataURL(file); } e.target.value = ''; }
    ...
    const startRecording = async () => {
        try {
            ...
            mediaRecorder.onstop = () => { const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/mp3' }); setAudioBlob(audioBlob); stream.getTracks().forEach(track => track.stop()); };
            mediaRecorder.start(); setIsRecording(true);
        } catch (error) { alert("Microphone access denied or error occurred."); }
    };
    const stopRecording = () => { if (mediaRecorderRef.current && isRecording) { mediaRecorderRef.current.stop(); setIsRecording(false); } };
    const cancelRecording = () => { if (mediaRecorderRef.current && isRecording) { mediaRecorderRef.current.stop(); setIsRecording(false); setAudioBlob(null); } };
    ...
    const handleBookingConfirmed = async (details: any) => {
        ...
        if(currentUser) { const persistedData = JSON.stringify(details); await supabase.from('chat_history').insert({ user_id: currentUser.id, provider_id: selectedChat ? selectedChat.id : null, role: 'BOT', text: persistedData }); }
        setTimeout(() => { scrollToBottom(); }, 300);
    }
    const handleBookFromProfile = (offer: Offer) => { setPreSelectedOffer(offer); setShowProfile(false); setShowBooking(true); }
            ...
            <div className="flex flex-col h-full bg-white relative">
                ...
                <div className="p-4 pb-2">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        {selectedCategory && (<button onClick={resetFilters} className="p-2 bg-red-100 rounded-full text-red-500 shadow-sm animate-fade-in"><RotateCcw size={18}/></button>)}
                    </div>
                    {renderFilterChips()}
                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <div className="flex flex-col items-end">
                                            {p.last_ad_at ? (<span className="text-[10px] text-green-600 font-bold">New Story</span>) : (<span className="text-[10px] text-gray-400">Online</span>)}
                                            {unreadCount > 0 && (<span className="bg-green-600 text-white text-[10px] font-bold px-1.5 h-4 min-w-[16px] rounded-full flex items-center justify-center mt-1">{unreadCount}</span>)}
                                        </div>
                                    </div>
                                    <p className="text-sm text-gray-500 flex items-center gap-1 truncate"><span className="font-semibold text-blue-600">{p.service_type}</span>{p.neighborhood && <span className="text-[10px] bg-gray-100 px-1 rounded text-gray-600">📍 {p.neighborhood}</span>}</p>
                                </div>
                            </div>
                        );
                    })}
    ...
    return (
        <div className="flex flex-col h-full bg-[#EFE7DD] relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#EFE7DD] relative">
                {messages.length === 0 && (<div className="flex flex-col items-center justify-center h-64 text-center opacity-60"><div className="bg-[#DCF8C6] p-4 rounded-full mb-2"><Globe className="text-green-600"/></div><p className="text-xs bg-[#FFF5C4] p-2 rounded shadow-sm text-gray-700">{selectedChat ? `You are now chatting with ${selectedChat.name}.` : t('welcomeMessage')}</p></div>)}
                {messages.map((m, i) => (
                    <div key={i} className={`flex mb-2 ${m.role === Role.USER ? 'justify-end' : 'justify-start'}`}>
                        {m.isWelcomeAd ? (
                            <div className="w-full max-w-[85%] bg-white rounded-xl overflow-hidden shadow-md border border-pink-200 mb-4 transform transition-all hover:scale-[1.01]">
                                <div className="bg-pink-50 p-2 flex items-center gap-2 border-b border-pink-100"><MessageSquare size={14} className="text-pink-600"/><span className="text-[10px] font-bold text-pink-600 uppercase tracking-wide">قصة جديدة (Story)</span></div>
                                {m.imageUrl && (<div className="w-full h-48 bg-gray-100 relative"><img src={m.imageUrl} className="w-full h-full object-cover" alt="Welcome"/></div>)}
                                <div className="p-3 bg-white"><p className="text-sm font-bold text-gray-800 leading-relaxed whitespace-pre-line">{m.text}</p></div>
                            </div>
                        ) : (
            ...
            {suggestions.length > 0 && !isLoading && !audioBlob && (
                <div className="p-2 bg-[#EFE7DD] flex gap-2 overflow-x-auto no-scrollbar">
                    {suggestions.map((chip, i) => (<button key={i} onClick={() => handleChipClick(chip)} className="bg-white border border-gray-200 text-blue-600 px-4 py-1.5 rounded-full text-xs font-bold shadow-sm whitespace-nowrap hover:bg-blue-50 transition-colors flex items-center gap-1"><Sparkles size={12} className="text-yellow-500"/> {chip}</button>))}
                </div>
            )}
            <div className="p-2 bg-white flex items-end gap-2 pb-safe border-t">
                {audioBlob ? (
                    <div className="flex-1 bg-red-100 rounded-full flex items-center px-4 py-2 justify-between animate-fade-in">
                        <div className="flex items-center gap-2 text-red-600 font-bold text-sm"><AudioWaveform size={20} className="animate-pulse"/><span>Audio Recorded</span></div>
                        <button onClick={cancelRecording} className="p-2 bg-white/50 rounded-full text-red-600 hover:bg-white"><X size={16}/></button>
                    </div>
                ) : (
                    <div className="flex-1 bg-gray-100 rounded-3xl flex items-center p-1.5 px-3">
                        ...
                        <textarea value={input} onChange={e => setInput(e.target.value)} placeholder={isRecording ? "Listening..." : t('inputPlaceholder')} rows={1} className="flex-1 bg-transparent border-none focus:ring-0 resize-none max-h-20 py-3 text-sm outline-none text-gray-800" onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); }}}/>
                        <button onClick={() => cameraInputRef.current?.click()} className="p-2 text-gray-400 hover:text-gray-600"><Camera size={20}/></button>
                    </div>
                )}
                <div className="relative" onTouchStart={(e) => { if (!input.trim() && !preview && !audioBlob) startRecording(); }} onTouchEnd={(e) => { if (isRecording) stopRecording(); }} onMouseDown={(e) => { if (!input.trim() && !preview && !audioBlob) startRecording(); }} onMouseUp={(e) => { if (isRecording) stopRecording(); }}>
                    <button onClick={() => { if(input.trim() || preview || audioBlob) handleSend(); }} className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg transition-all ${input.trim() || preview || audioBlob ? 'bg-blue-600 text-white' : (isRecording ? 'bg-red-500 text-white scale-110' : 'bg-green-500 text-white')}`}>
                        {input.trim() || preview || audioBlob ? <Send size={20} className={language === 'ar' ? 'rotate-180 ml-1' : 'mr-1'}/> : (isRecording ? <StopCircle size={24} className="animate-pulse"/> : <Mic size={24}/>)}
                    </button>
                </div>
            </div>
