...
const v26UpdateSQL = `-- V26: Fix Directory & Notifications
-- 1. Ensure Follows Table exists and is public
...
-- 2. Ensure Notifications are public
ALTER TABLE public.provider_notifications ENABLE ROW LEVEL SECURITY;
...
const v27UpdateSQL = `-- V27: Enhanced Job Board (Souq Choghl)
-- 1. Update job_posts with new columns
ALTER TABLE public.job_posts ADD COLUMN IF NOT EXISTS category text DEFAULT 'cat_other';
ALTER TABLE public.job_posts ADD COLUMN IF NOT EXISTS post_type text DEFAULT 'SEEKER'; -- 'SEEKER' or 'EMPLOYER'
ALTER TABLE public.job_posts ADD COLUMN IF NOT EXISTS image_url text NULL;
...
CREATE TABLE IF NOT EXISTS public.job_comments (
  ...
  CONSTRAINT job_comments_job_id_fkey FOREIGN KEY (job_id) REFERENCES job_posts(id) ON DELETE CASCADE
) TABLESPACE pg_default;
...
DROP POLICY IF EXISTS "Public Jobs" ON public.job_posts;
CREATE POLICY "Public Jobs" ON public.job_posts FOR ALL USING (true) WITH CHECK (true);

NOTIFY pgrst, 'reload config';`;

const v28UpdateSQL = `-- V28: Force Enable Product Deletion (Store Fix)
-- 1. Re-apply permissions with explicit DELETE grant for ALL roles (simplest for this app)
DROP POLICY IF EXISTS "Public Access Products" ON public.products;
CREATE POLICY "Public Access Products" ON public.products FOR ALL USING (true) WITH CHECK (true);
        ...
        <div className="p-6 h-[calc(100vh-65px)] overflow-y-auto">
          ...
          <div className="mb-8 p-4 border-l-4 border-purple-500 bg-purple-50 dark:bg-purple-900/20 rounded-r-lg shadow-lg animate-fade-in">
             <h3 className="text-xl font-bold text-purple-600 dark:text-purple-400 flex items-center gap-2 mb-4">
                <Lock /> V28 Update: Fix Delete Permissions
             </h3>
             <p className="mb-2 text-sm text-gray-600 dark:text-gray-400">Run this if you can't delete products from the Store.</p>
             <CodeBlock title="V28 Fix Delete" code={v28UpdateSQL.trim()} />
          </div>
          ...
          <div className="mb-8 p-4 border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/20 rounded-r-lg shadow-lg animate-fade-in">
             <h3 className="text-xl font-bold text-blue-600 dark:text-blue-400 flex items-center gap-2 mb-4">
                <Briefcase /> V27 Update: Advanced Job Board
             </h3>
             <p className="mb-2 text-sm text-gray-600 dark:text-gray-400">Run this to enable Job Categories, Seeker/Employer types, and Comments.</p>
             <CodeBlock title="V27 Job Board Update" code={v27UpdateSQL.trim()} />
          </div>
